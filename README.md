...
#### clientId, tennantId
This is a new props that allow you do multiple configurations for one app
...
// Config object to be passed to Msal on creation
        ...
        authorities: [
            {
                type: 'B2C',
                audience: 'AzureADandPersonalMicrosoftAccount',
                authorityUrl: `https://${config.azureB2C.knownAuthority}/tfp/${config.azureB2C.tenant}/${config.azureB2C.userFlows.signUpSignIn}`,
                cloudInstance: 'MSALAzurePublicCloudInstance',
                default: true
            }
        ],
    ...
    const msalAADConfig = {
        authorities: [
            {
                ...
                authorityUrl: `https://login.microsoftonline.com/${config.azureAAD.tenantId}`,
                cloudInstance: 'MSALAzurePublicCloudInstance',
                default: true
            }
        ],
        ...
        brokerRedirectUri: false,
        accountMode: 'SINGLE',
        clientId:config.azureAAD.clientId,
        tenantId:config.azureAAD.tenantId,
        scopes: [...config.azureAAD.scopes]
    };
    //now you can pass one of the above config to msalInit method
```
